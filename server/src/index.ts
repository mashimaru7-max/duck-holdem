...
import {applyAction,continueAfterHand,expireTurn,newPlayer,newRoom,startHand,viewFor,type Player,type Room} from './engine.js';
...
function broadcast(room:Room){for(const player of room.players)for(const ws of sockets.get(player.id)??[])send(ws,{type:'state',state:viewFor(room,player)});broadcastLobby();}
...
function removeFromRoom(room:Room,player:Player){
  ...
  if(room.players.length===0){rooms.delete(room.code);broadcastLobby();return;}
...
app.get('/ws',{websocket:true},socket=>{
  ...
  socket.on('message',(raw:Buffer)=>{
    ...
    try{
      if(msg.type==='create_room'){
        ...
        const player=newPlayer(msg.nickname.trim().slice(0,12),msg.sessionId||randomUUID(),1);const room=newRoom(player,code());rooms.set(room.code,room);current={room,player};sessions.set(player.sessionId,current);
        ...
        else{if(current&&(current.room.status==='WAITING'||current.room.status==='HAND_END'))removeFromRoom(current.room,current.player);const seat=Array.from({length:8},(_,i)=>i+1).find(value=>!room.players.some(player=>player.seat===value))!;const player=newPlayer(msg.nickname.trim().slice(0,12),msg.sessionId,seat);room.players.push(player);current={room,player};sessions.set(player.sessionId,current);room.version++;}
      }else{
  ...
  socket.on('close',()=>{clients.delete(socket);if(current){sockets.get(current.player.id)?.delete(socket);current.player.connected=false;current.room.version++;broadcast(current.room);}});
});
