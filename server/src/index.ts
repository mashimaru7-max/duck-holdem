...
import type {ClientMessage,ServerMessage} from '@duck-holdem/shared';import {applyAction,newPlayer,newRoom,startHand,viewFor,type Player,type Room} from './engine.js';
...
app.get('/ws',{websocket:true},(socket)=>{let current:{room:Room;player:Player}|undefined;
  socket.on('message',(raw:Buffer)=>{let msg:ClientMessage;try{msg=JSON.parse(raw.toString());}catch{return send(socket,{type:'error',code:'BAD_JSON',message:'잘못된 요청입니다.'});}
    try{
      ...
      else {if(!current)throw new Error('먼저 방에 참가하세요.');const {room,player}=current;if(msg.type==='ready'){player.ready=msg.ready;room.version++;}else if(msg.type==='start'){if(room.hostId!==player.id)throw new Error('방장만 시작할 수 있습니다.');startHand(room);}else applyAction(room,player.id,msg.action,msg.commandId,msg.expectedVersion);}
