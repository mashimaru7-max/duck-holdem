...
export interface Room {code:string;hostId:string;status:Street;version:number;players:Player[];deck:Card[];board:Card[];dealerSeat:number;sbSeat:number;bbSeat:number;actionSeat?:number;pot:number;currentBet:number;minRaise:number;handId?:string;deadlineAt?:number;processed:Set<string>}
...
export function startHand(room:Room,random=Math.random){
  ...
  room.handId=randomUUID(); room.status='PREFLOP'; room.version++; room.deck=shuffle(createDeck(),random);room.board=[];room.pot=0;room.currentBet=2;room.minRaise=2;
...
function awardUncontested(room:Room){const winner=contenders(room)[0];winner.stack+=room.players.reduce((s,p)=>s+p.totalContribution,0);winner.lastAction='승리';room.pot=0;room.status='HAND_END';room.actionSeat=undefined;room.deadlineAt=undefined;}
function settle(room:Room){
  ...
  for(const level of levels){const contributors=room.players.filter(p=>p.totalContribution>=level);const amount=(level-previous)*contributors.length;const eligible=contributors.filter(p=>!p.folded);if(!eligible.length){previous=level;continue;}
    const scored=eligible.map(p=>({p,s:evaluate([...p.holeCards,...room.board])}));const best=scored.map(x=>x.s).sort((a,b)=>compareScore(b,a))[0];const winners=scored.filter(x=>compareScore(x.s,best)===0).map(x=>x.p).sort((a,b)=>a.seat-b.seat);const share=Math.floor(amount/winners.length);let rem=amount%winners.length;winners.forEach(w=>{w.stack+=share+(rem-->0?1:0);w.lastAction=`승리 · ${best.name}`;});previous=level;
  } room.pot=0;room.status='HAND_END';room.actionSeat=undefined;room.deadlineAt=undefined;room.players.forEach(p=>p.holeCards=p.holeCards);
}
